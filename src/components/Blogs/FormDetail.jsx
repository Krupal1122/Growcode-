...
const BlogDetail = () => {
  ...
  const readingTime = ' '; // Update with actual reading time logic if needed
  ...
  // Determine the back link based on the source
  const backLink = state?.from === 'admin' ? '/admin/documents' : '/';
    ...
    <div className="bg-white text-gray-800 font-sans w-screen min-h-screen flex">
      <div className="flex w-full h-full mt-16">
        ...
        <div className="flex-1 h-screen overflow-y-auto p-6" ref={contentRef}>
          ...
          >
            ← Back to {state?.from === 'admin' ? 'Forms' : 'Blogs'}
          </Link>
